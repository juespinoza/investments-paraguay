...
export function Button(props: ButtonProps) {
  ...
  const variants: Record<ButtonVariant, string> = {
    primary:
      "bg-(--carbon) text-(--ivory) shadow-[0_16px_36px_rgba(10,10,10,0.18)] hover:-translate-y-0.5 hover:bg-(--gold) hover:text-(--carbon)",
    secondary:
      "border border-(--line) bg-[rgba(250,250,248,0.72)] text-primary hover:-translate-y-0.5 hover:border-(--gold) hover:bg-(--stone)",
  };
  ...
  if ("href" in rest) {
    const { href, target, rel, prefetch, ...linkProps } = rest as Omit<
      ButtonAsLink,
      keyof Common
    >;

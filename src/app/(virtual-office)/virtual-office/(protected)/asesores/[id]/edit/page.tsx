...
import { isAdmin, isAdvisor } from "@/lib/auth/permissions";
...
export default async function EditAdvisorPage({ params }: PageProps) {
  ...
  return (
    <div>
      <PageHeader
        title={isAdvisorSelf ? "Mi landing pública" : "Editar asesor"}
        description={
          isAdvisorSelf
            ? "Mantén tu perfil público, tu propuesta de valor y tus propiedades destacadas siempre actualizadas."
            : "Actualizá los datos del asesor."
        }
        actions={
          <Link
          ...
          >
            {isAdvisorSelf ? "Crear propiedad" : "Crear propiedad para este asesor"}
          </Link>
        }
      />
